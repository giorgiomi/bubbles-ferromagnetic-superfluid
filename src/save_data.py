...
# for fs in [0]: # only the first seq
    ...
    for ei, ai in enumerate((seqs[fs])):
        ...
        np.savetxt(f"data/day_{fs}/seq_{ei}/center.csv", b_center, delimiter=',')
        np.savetxt(f"data/day_{fs}/seq_{ei}/sizeADV.csv", b_sizeADV, delimiter=',')
        np.savetxt(f"data/day_{fs}/seq_{ei}/magnetization.csv", M, delimiter=',')
