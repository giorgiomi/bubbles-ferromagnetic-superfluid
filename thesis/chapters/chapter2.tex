...
\begin{figure}[h!]
    ...
    \caption{Fit parameters $\ell_1$, $\Delta$ (off) and $\ell_2$ of shots grouped by size.}
    \label{fig:param_size_inside}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[width = \textwidth]{figures/chap2/param_slope_inside.png}
    \caption{Fit parameters $\ell_1$, $\Delta$ (off) and $\ell_2$ of shots grouped by slope.}
    \label{fig:param_slope_inside}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[width = \textwidth]{figures/chap2/fit_omega_inside.png}
    \caption{ACF average profiles of shots grouped by $\Omega_R$ (solid lines) and fitted with the damped cosine (dotted lines).}
    \label{fig:fit_omega_inside}
\end{figure}
The time dependence seems to be the most noticeable. From Fig.\ \ref{fig:param_time_inside}, the first parameter $\ell_1$ is decreasing with time, while the second parameter $\Delta$ is increasing. This behaviour could mean that the magnetization in the bubble loses its coherence with time.
