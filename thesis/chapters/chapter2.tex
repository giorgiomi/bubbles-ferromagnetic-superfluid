...
Now, what we shall do is to compute the FFT and the ACF on the data, taking care of the fact that it is probably better to separate the inside region (the bubble) from the outside one and look for periodic signals in both. We will also compute the transforms on the no-bubble shots. An example of the inside region computation is presented in Fig.\ \ref{fig:inside_00}.

\begin{figure}[t!]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/chap2/inside_fft_acf_day_0_seq_0.png}
    \caption{Example of FFT and ACF calculated in a sequence. The values for each shot are shown in the left plots, while the averages are on the right.}
    \label{fig:inside_00}
\end{figure}
